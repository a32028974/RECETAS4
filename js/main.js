// /js/main.js — v2025-11-03 • sin PDF/Telegram • fechas OK (ISO en <input type="date">)
...
import { cargarFechaHoy } from './fechaHoy.js'; // solo usamos esta; la fecha de retiro la calculamos acá
...
/* ============================================================
   PROGRESO (solo validación → guardado → listo)
   ============================================================ */
...
/* ============================================================
   FECHAS • utilidades
   - parseArgDate: lee dd/mm/aaaa (como tu “Fecha que encarga”)
   - toLocalISO:  yyyy-MM-dd para <input type="date"> (evita warning)
   ============================================================ */
...
/* ============================================================
   Cálculo de “Fecha que retira (estimada)”
   - Base: “Fecha que encarga” (dd/mm/aaaa) o hoy si está vacío
   - Modo: Stock=+7, Urgente=+3, Laboratorio=+15
   - Output al input: ISO si type="date", sino dd/mm/aaaa
   ============================================================ */
function recalcularFechaRetiro(){
  ...
  if (fechaOut.type === 'date') {
    fechaOut.value = toLocalISO(result);      // evita warning del navegador
  } else {
    // fallback si fuera input de texto
    const dd=String(result.getDate()).padStart(2,'0');
    const mm=String(result.getMonth()+1).padStart(2,'0');
    const yy=String(result.getFullYear());
    fechaOut.value = `${dd}/${mm}/${yy}`;
  }
}

/* ============================================================
   VALIDACIONES Y HELPERS (respetan tu flujo; no tocan guardar.js)
   ============================================================ */
function validarDistanciaFocal(){ return typeof window.__validarDF === 'function' ? window.__validarDF() : true; }
function validarEjesRequeridos(){ return typeof window.__validarEjes === 'function' ? window.__validarEjes() : true; }
function setupGraduacionesSelects(){ if(typeof window.__setupGrads==='function') window.__setupGrads(); }
function setupCalculos(){ if(typeof window.__setupTotals==='function') window.__setupTotals(); }
...
function generarNumeroTrabajoDesdeTelefono(){
  // si ya tenés la función en numeroTrabajo.js, dejá que se encargue;
  if (typeof obtenerNumeroTrabajoDesdeTelefono === 'function') return obtenerNumeroTrabajoDesdeTelefono();
  // fallback mínimo
  const tel=$('telefono')?.value.replace(/\D/g,'')||'';
  if(!tel) return;
  const base='5'+tel;
  const caja=$('numero_trabajo');
  if(caja && !caja.value.trim()) caja.value=base.slice(0,10);
}
...
document.addEventListener('DOMContentLoaded', () => {
  // Cámara + Galería (se mantiene)
  initPhotoPack();

  // Fecha que encarga (tu función original)
  cargarFechaHoy();

  // Fecha que retira (estimada)
  ...
  // Graduaciones / Totales (dejamos ganchos a tu lógica existente)
  ...
  if(btnEdit){
    btnEdit.addEventListener('click', async()=>{
      ...
      if(typeof window.__cargarTrabajoAnterior==='function') await window.__cargarTrabajoAnterior(nro);
    });
  }
